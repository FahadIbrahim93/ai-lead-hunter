...
NICHE_OFFERS = [
    ...
    (("dental", "dentist"),
     "an AI appointment agent that answers treatment enquiries with instant price guidance, books patients into real-time slots at the right branch, and sends recall reminders so no patient is lost"),
    (("diagnostic", "healthcare", "clinic", "hospital", "medical"),
     "an AI appointment agent that books tests and consultations with instant confirmation, delivers report-ready notifications, and follows up with patients automatically"),
    (("ielts", "education", "coaching", "academy", "english", "training"),
     "an AI admission agent that answers course enquiries instantly, recommends the right course from each student's target score, and books free trial classes with automated reminders"),
    (("event", "wedding", "concert", "activation"),
     "an AI enquiry agent that acknowledges every event request within seconds, qualifies the event type and budget, and sends tailored proposals while your team is still on site"),
    (("photography", "cinematography", "photo studio"),
     "an AI booking agent that answers package and pricing enquiries instantly, checks your shooting calendar, and locks in bookings with automated contracts and reminders"),
    (("restaurant", "food", "cafe", "catering"),
     "an AI order agent that takes orders over WhatsApp, confirms menu items and delivery ETA, and sends status updates so your staff never misses a call during peak hours"),
    (("bakery", "cake", "pastry"),
     "an AI order agent that captures custom cake requests with design details and delivery dates over WhatsApp, confirms pricing instantly, and sends order-status updates automatically"),
    (("gym", "fitness", "yoga", "crossfit"),
     "an AI membership agent that answers pricing and class enquiries instantly, books trial sessions, and sends renewal reminders so members never lapse unnoticed"),
    (("salon", "spa", "beauty", "parlour", "parlor"),
     "an AI booking agent that answers service and pricing enquiries, books appointments into your stylists' real-time availability, and sends pre-visit reminders to cut no-shows"),
    (("legal", "law", "advocate", "attorney", "chamber"),
     "an AI intake agent that triages every case enquiry, collects the documents you need before the first meeting, and schedules consultations with automated reminders"),
    (("travel", "hajj", "umrah", "visa", "tour", "airline"),
     "an AI enquiry agent that answers package and visa questions instantly, tracks every application's status, and sends document checklists and deadline reminders automatically"),
    (("packaging", "corrugated", "box", "printing"),
     "an AI quote agent that captures spec requirements (size, quantity, material), generates instant ballpark quotes, and routes serious B2B enquiries straight to your sales team"),
    (("courier", "parcel", "logistics", "delivery service"),
     "an AI tracking agent that answers 'where is my parcel?' queries automatically 24/7, books new shipments over WhatsApp, and sends delivery confirmations without staff touching a keyboard"),
    ...
    (("marketing", "advertising", "branding"),
     "an AI lead-qualification agent that answers inbound enquiries, scores each prospect, and books discovery calls so your team only talks to serious clients"),
